...
MVAMETPairProducer::MVAMETPairProducer(const edm::ParameterSet& cfg) 
   : mvaMEtAlgo_(cfg),mvaJetIdAlgo_(cfg),mvaMEtAlgo_isInitialized_(false)
...
{ 
  ...
  if (verbosity_) {
    ...
    for (unsigned j = 0; j < leptonInfo[i].size(); ++j) {
     std::cout <<"Lept"<<(j+1)<<": ";
     std::cout <<"pt: "<< leptonInfo[i][j].p4_.pt() << "   ";
     std::cout <<"eta: "<< leptonInfo[i][j].p4_.eta() << "   ";
     std::cout <<"phi: "<< leptonInfo[i][j].p4_.phi() << "   ";
     std::cout <<"mass: "<< leptonInfo[i][j].p4_.mass() << "   ";
